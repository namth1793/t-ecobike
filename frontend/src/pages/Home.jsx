  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-dark-2">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center">
        ...
        <p className="text-2xl sm:text-3xl text-gray-200 font-medium mb-4">No License Needed</p>
        <p className="text-gray-400 text-lg max-w-2xl mx-auto mb-10 leading-relaxed">
          Travel up to <span className="text-primary-400 font-semibold">60km</span>, swap batteries in{' '}
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          ...
          <a href="#scooter-details" className="btn-outline text-base px-8 py-4 text-white border-white/30 hover:border-primary-400 hover:bg-primary-500/10">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center max-w-5xl mx-auto">
          <div className={`relative transition-all duration-700 delay-150 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
            <div className="rounded-3xl overflow-hidden shadow-2xl group">
              <img src="/assets/intro.jpg" alt="VinFast Evo Grand Lite" className="w-full h-80 object-cover group-hover:scale-105 transition-transform duration-700" />
            </div>
          ...
          <div className={`pt-6 lg:pt-0 transition-all duration-700 delay-200 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}>
            ...
            <p className="text-gray-500 text-sm mb-6">The perfect choice for exploring Da Nang. Stylish design, no license needed, eco-friendly.</p>
            ...
            <div className="flex gap-3">
              <Link to="/booking?scooter=1" className="btn-primary flex-1 justify-center text-sm">Book Now</Link>
              <a href="#scooter-details" className="btn-outline flex-1 justify-center text-sm">View Details</a>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-16 grid grid-cols-3 gap-4 max-w-5xl mx-auto transition-all duration-700 delay-300 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {[
            ...
            <div key={i} className="rounded-2xl overflow-hidden shadow-lg group cursor-pointer">
              <img src={src} alt={`T-EcoBike photo ${i + 1}`} className="w-full h-52 object-cover group-hover:scale-105 transition-transform duration-500" />
            </div>
          ))}
        </div>
      </div>
...
function ElectricMotorbikeSection() {
  ...
  return (
    <section id="scooter-details" ref={ref} className="py-20 lg:py-28 bg-dark-2">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center mb-14 transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <span className="text-primary-400 text-sm font-semibold uppercase tracking-wider">VinFast Evo Grand Lite</span>
          <h2 className="text-3xl md:text-4xl font-bold text-white mt-2">Specs & Pricing</h2>
          <p className="text-gray-400 mt-3">Everything you need to know before you ride</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          {/* Left: specs + what's included */}
          <div>
            <div className="text-white font-semibold text-xs uppercase tracking-wider mb-4">Technical Specifications</div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
              {specs.map((s, i) => (
                <div
                  key={s.label}
                  className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center hover:border-primary-500/40 transition-all duration-300"
                  style={{ transitionDelay: `${i * 50}ms`, opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(16px)', transition: 'all 0.5s' }}
                >
            ...
            <div className="text-white font-semibold text-xs uppercase tracking-wider mb-4">What's Included</div>
            <div className="grid grid-cols-2 gap-3">
              {included.map((item, i) => (
                <div
                  key={item.title}
                  className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl p-4 hover:border-primary-500/40 transition-colors"
                  style={{ transitionDelay: `${i * 80}ms`, opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(16px)', transition: 'all 0.5s' }}
                >
          ...
          <div className={`transition-all duration-700 delay-200 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}>
            <div className="text-white font-semibold text-xs uppercase tracking-wider mb-4">Price Calculator</div>
            <div className="bg-white/5 border border-white/10 rounded-3xl p-7">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm text-gray-300">Number of days</span>
                <span className="text-2xl font-extrabold text-primary-400">{days} {days === 1 ? 'day' : 'days'}</span>
              </div>
              <input
                type="range" min={1} max={30} value={days}
                onChange={e => setDays(Number(e.target.value))}
                className="w-full h-2 bg-white/10 rounded-full appearance-none cursor-pointer accent-green-500 mb-6"
              />
              <div className="grid grid-cols-3 gap-3 mb-5">
                <div className="bg-white/10 rounded-2xl p-4 text-center">
                  <div className="text-xs text-gray-400 mb-1">Per day</div>
                  <div className="text-lg font-extrabold text-white">{perDay.toLocaleString('vi-VN')}₫</div>
                </div>
                <div className="bg-white/10 rounded-2xl p-4 text-center">
                  <div className="text-xs text-gray-400 mb-1">Days</div>
                  <div className="text-lg font-extrabold text-white">{days}</div>
                </div>
                <div className="bg-primary-500 rounded-2xl p-4 text-center shadow-lg shadow-primary-500/30">
                  <div className="text-xs text-primary-100 mb-1">Total</div>
                  <div className="text-lg font-extrabold text-white">{price.toLocaleString('vi-VN')}₫</div>
                </div>
              </div>
              {days >= 2 && (
                <p className="text-center text-primary-400 text-sm font-medium mb-5">
                  🎉 Save {(days * 50000).toLocaleString('vi-VN')}₫ vs 1-day rate!
                </p>
              )}
              <Link to="/booking?scooter=1" className="btn-primary w-full justify-center">
    ...
    <section className="py-20 lg:py-28 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {benefits.map((b, i) => (
            <div key={i} className="group p-7 rounded-2xl border border-gray-100 hover:border-primary-200 hover:shadow-lg hover:shadow-primary-500/10 transition-all duration-300 hover:-translate-y-1">
...
const tips = [
  ...
  { q: 'What to do when the battery is low', a: 'When the battery is running low, find the nearest swap station via the VinFast app or call us at +84 905 659 886 for directions to the closest station within 10km.' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white mt-2 mb-4">Safety & Tips</h2>
            <p className="text-gray-400 text-lg mb-8 leading-relaxed">Safety is our top priority. Please read and follow these guidelines for a safe and enjoyable ride.</p>
            <div className="space-y-3">
              {tips.map((tip, i) => (
                <div key={i} className="border border-white/10 rounded-xl overflow-hidden">
                  ...
                  >
                    <span className={`font-medium text-sm ${open === i ? 'text-primary-400' : 'text-gray-200'}`}>{tip.q}</span>
                    <svg className={`w-5 h-5 text-gray-400 transition-transform flex-shrink-0 ml-4 ${open === i ? 'rotate-180 text-primary-400' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>
                  {open === i && (
          ...
          <div className="relative">
            <div className="rounded-2xl overflow-hidden shadow-2xl shadow-primary-500/10">
              <img src="/assets/z7848049053773_91b9c21f271cf11d75540df23129fb02.jpg" alt="Safe riding" className="w-full h-80 lg:h-96 object-cover" />
...
const steps = [
  ...
  { step: '02', title: 'Submit Documents', desc: "Provide a valid ID or passport. No driver's license required — just a valid identity document!", icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" /> },
  { step: '03', title: 'Free Delivery', desc: 'Your scooter is delivered free of charge to your hotel or any central Da Nang location. No pick-up needed!', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /> },
  { step: '04', title: 'Ride & Explore', desc: 'Pay by cash or credit card (3% fee). Explore Da Nang your way!', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /> }
];
    ...
    <section className="py-20 lg:py-28 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <p className="section-subtitle">Just 4 simple steps to get your electric scooter for the journey</p>
        </div>
        ...
        <div className="text-center mt-12">
          <Link to="/booking" className="btn-primary text-base px-10 py-4">Start Your Journey →</Link>
        </div>
      </div>
    </section>
  );
...
/* ─── About Us ─── */
const team = [
  { name: 'Minh Vu', role: 'Founder & CEO', img: 'https://i.pravatar.cc/200?img=12', desc: 'Founder with a passion for sustainable green tourism in Da Nang.' },
  { name: 'Lan Anh', role: 'Operations Manager', img: 'https://i.pravatar.cc/200?img=25', desc: 'Coordinates daily operations to ensure a perfect customer experience.' },
  { name: 'Tuan Nguyen', role: 'Head Mechanic', img: 'https://i.pravatar.cc/200?img=15', desc: 'Lead technician maintaining and inspecting the entire scooter fleet.' }
];
...
function AboutSection() {
  ...
  return (
    <section id="about" ref={ref} className="py-20 lg:py-28 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center mb-14 transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <span className="text-primary-600 text-sm font-semibold uppercase tracking-wider">Who We Are</span>
          <h2 className="section-title mt-2">About T-EcoBike</h2>
          <p className="section-subtitle">Pioneering eco-friendly travel in Da Nang since 2019</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className="bg-gray-50 rounded-3xl p-8">
            ...
            <h3 className="text-xl font-bold text-dark-2 mb-3">Our Mission</h3>
            <p className="text-gray-500 leading-relaxed text-sm">To provide premium electric scooter rentals that help travellers explore Da Nang in the most eco-friendly way — delivering convenience, safety and unforgettable experiences for every customer.</p>
          </div>
          <div className="bg-primary-600 rounded-3xl p-8 text-white">
            ...
            <p className="text-primary-100 leading-relaxed text-sm">To become the leading electric scooter rental service in Central Vietnam, contributing to reduced environmental pollution and promoting sustainable tourism in Da Nang.</p>
          </div>
        </div>

        {/* Stats */}
        <div className="bg-primary-500 rounded-3xl py-10 px-8 mb-12">
        ...
        <div className="text-center mb-10">
          <h3 className="text-2xl font-bold text-dark-2">Meet Our Team</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-3xl mx-auto">
          {team.map((m, i) => (
            <div
              ...
              style={{ transitionDelay: `${i * 100}ms`, opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(20px)', transition: 'all 0.6s' }}
            >
              <img src={m.img} alt={m.name} className="w-20 h-20 rounded-2xl object-cover mx-auto mb-3 group-hover:scale-105 transition-transform shadow-lg" />
    ...
    <section className="py-20 lg:py-28 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <p className="section-subtitle">Thousands of travellers trust and love riding with T-EcoBike</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-3xl p-8 md:p-12 text-center relative shadow-sm">
          ...
          <div className="flex justify-center gap-2 mt-6">
            {testimonials.map((_, i) => (
              <button key={i} onClick={() => setIdx(i)} className={`rounded-full transition-all ${i === idx ? 'w-8 h-2.5 bg-primary-500' : 'w-2.5 h-2.5 bg-gray-300 hover:bg-gray-400'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-10">
          {testimonials.map((t2, i) => (
            <button key={t2.id} onClick={() => setIdx(i)} className={`p-4 rounded-xl text-left transition-all ${i === idx ? 'bg-primary-50 border-2 border-primary-300' : 'bg-white border-2 border-transparent hover:border-gray-200'}`}>
...
function ContactSection() {
  ...
  return (
    <section id="contact" className="py-20 lg:py-28 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <span className="text-primary-600 text-sm font-semibold uppercase tracking-wider">Contact Us</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="space-y-4">
            {[
              {
                ...
                title: 'Hotline', text: '+84 905 659 886', href: 'tel:+84905659886', color: 'blue'
              },
              {
                icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />,
                title: 'Email', text: 'truongvunguyen1991@gmail.com', href: 'mailto:truongvunguyen1991@gmail.com', color: 'green'
              },
              {
                ...
                title: 'Address', text: '135 Tran Nhan Tong, Hoi An Dong, Da Nang', color: 'orange'
              }
            ].map(c => (
              <div key={c.title} className="flex gap-4 p-4 bg-gray-50 rounded-xl hover:bg-primary-50 transition-colors">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${c.color === 'green' ? 'bg-primary-100' : c.color === 'blue' ? 'bg-blue-100' : 'bg-orange-100'}`}>
                  <svg className={`w-5 h-5 ${c.color === 'green' ? 'text-primary-600' : c.color === 'blue' ? 'text-blue-600' : 'text-orange-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {c.icon}
                  </svg>
                </div>
                <div>
                  ...
                  {c.href
                    ? <a href={c.href} className="text-sm text-gray-500 hover:text-primary-600 transition-colors">{c.text}</a>
            ...
            <div className="pt-2 flex gap-3 flex-wrap">
              ...
              <a href="tel:+84905659886" className="flex items-center gap-2 bg-yellow-400 hover:bg-yellow-500 text-sm px-4 py-2 rounded-full transition-colors font-bold" style={{ color: '#3B1C1C' }}>
                <span>KT</span>
                KakaoTalk
              </a>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-gray-50 rounded-3xl p-8">
              <h3 className="text-lg font-bold text-dark-2 mb-6">Send Us a Message</h3>

              {status === 'success' && (
                <div className="bg-primary-50 border border-primary-200 text-primary-700 rounded-xl p-4 mb-6 flex items-center gap-3 text-sm">
              ...
              <form onSubmit={submit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input name="full_name" value={form.full_name} onChange={handle} required placeholder="John Smith" className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Phone</label>
                    <input name="phone" value={form.phone} onChange={handle} placeholder="+84 905 659 886" className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Email</label>
                  <input type="email" name="email" value={form.email} onChange={handle} placeholder="your@email.com" className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Subject</label>
                  <select name="subject" value={form.subject} onChange={handle} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all">
                ...
                <div>
                  ...
                  <textarea name="message" value={form.message} onChange={handle} required rows={4} placeholder="How can we help you?" className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all resize-none" />
                </div>
  ...
  return (
    <section className="py-20 bg-gradient-to-r from-primary-600 via-primary-500 to-primary-400">
      <div className="max-w-4xl mx-auto px-4 text-center">
        <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-4">Ready to Ride the Future?</h2>
        <p className="text-primary-100 text-lg mb-8">Book today — Free delivery, no license needed, lightning-fast battery swap!</p>
        <div className="flex flex-wrap gap-4 justify-center">
          ...
          <a href="https://wa.me/84905659886" target="_blank" rel="noopener noreferrer" className="bg-primary-700/50 hover:bg-primary-700 text-white font-semibold px-8 py-4 rounded-full transition-all inline-flex items-center gap-2">
